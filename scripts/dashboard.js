                ...
                if (data.success) {
                   ...
                   // Handle image previews
                    ...
                    if (data.collateral_files) {
                        ...
                        collateralPreviewContainer.innerHTML = ''; // Clear previous images
                    
                        // Ensure collateral_files is an array
                        let collateralFiles = Array.isArray(data.collateral_files)
                            ? data.collateral_files
                            : data.collateral_files.split(','); // 🔹 Properly split comma-separated paths
                    
                        console.log('Collateral files:', collateralFiles);
                    
                        collateralFiles.forEach(file => {
                            let filePath = file.trim();
                    
                            // Ensure the filePath is valid before creating an image
                            if (filePath) {
                                const collateralImg = document.createElement('img');
                                collateralImg.src = filePath; // ✅ Set correct image path
